...
class DmSimulatorPy_Base(BackendV1):
    """ Base Class for Python implementation of a Density Matrix simulator.
    ...
    def __init__(self, configuration=None, provider=None):
        ...
        self._two_qubit_gate_qasm = None
        self._two_qubit_gate_descriptor = None # This is the string that would be printed if  SHOW_PARTITION is True
        self._two_qubit_gate_partition_name = None

        # Define which errors to be applied
        self._decoherence_and_amp_decay_applied = True
        self._depolarization_applied = True
        ...
        self._tsp_model_error = (
            None  # [<cos(fluctuation)>, mean]  , Transition selective pulse error
        )
        self._thermal_factor = None  # p
        self._decoherence_factor = None  # f
        self._decay_factor = None  # g
        self._depolarization_factor = (
    ...
    def _set_options(self, qobj_config=None, backend_options=None):
        ...
        if "rotation_error" in backend_options:
            if type(backend_options["rotation_error"]) != dict or not all(
                x in ["rx", "ry", "rz"] for x in backend_options["rotation_error"]
            ):
                raise BasicAerError(
                    "Error! Incorrect Rotation Error parameters, Expected argument : A dict with rotation gate as key and a list of 2 reals ranging between 0 and 1 both inclusive as their values."
                )
            else:
        ...
        if "tsp_model_error" in backend_options:
            if (
                type(backend_options["tsp_model_error"]) != list
                or len(backend_options["tsp_model_error"]) != 2
                or backend_options["tsp_model_error"][0] > 1
                or backend_options["tsp_model_error"][1] > 1
            ):
                raise BasicAerError(
                    "Error! Incorrect transition model error parameter, Expected argument : A list of 2 reals ranging between 0 and 1 both inclusive."
                )
            else:
    ...
    def _initialize_errors(self):
        ...
        self._error_params.update(
            {
                "memory": {
                    "thermalization": self._thermal_factor,
        ...
        self._error_params.update(
            {
                "measurement": self._depolarization_factor,
    ...
    def _validate_initial_densitymatrix(self):
        ...
        if length != required_dim:
            raise BasicAerError(
                "initial densitymatrix is incorrect length: " + "{} != {}".formarequired_dim
            )
    ...
    def _add_decoherence_and_amp_decay(self, level, f, p, g):
        """Apply decoherence transformation and amplitude decay transformation independently
        ...
        # Applying Decoherence and Decay
    ...
    def _add_ensemble_measure(self, basis, add_param, err_param):
        ...
        measure_ind = [
            x for x in itertools.product(supplement_data[basis], repeat=self._number_of_qubits)
        ]
        ...
        if basis != "N":
            ...
            for i in range(self._number_of_qubits - 1):
                operator_mes = np.kron(
                    np.array([[1, n[0], n[1], n[2]], [1, -n[0], -n[1], -n[2]]]), operator_mes
                )
        ...
        if self.COMPARE and self.FILE_EXIST:
            self._fidelity = self._state_overlap(
                self._density_matrix_stored,
                np.reshape(self._densitymatrix, 4**self._number_of_qubits),
            )
    ...
    def _add_partial_measure(
        self, measured_qubits, cmembits, cregbits, err_param, basis, add_param=None
    ):
        ...
        supplement_data = {
            "X": [self._add_qasm_measure_X, [0, 1]],
        ...
        probabilities = np.reshape(
            np.sum(np.reshape(prob_ensemble, self._number_of_qubits * [2]), axis=tuple(axis)),
            2**num_measured,
        )
    ...
    def _add_qasm_measure_N(
        self, qubit, cmembit, cregbit=None, n=np.array([0.0, 0.0, 1.0]), err_param=1.0
    ):
        ...
        temp = (
            n[0] * self._densitymatrix[:, 1, :]
            + n[1] * self._densitymatrix[:, 2, :]
    ...
    def _add_bell_basis_measure(self, qubit_1, qubit_2, err_param):
        ...
        self._densitymatrix = np.reshape(
            self._densitymatrix,
            (4 ** (self._number_of_qubits - q_2 - 1), 4, 4 ** (q_2 - q_1 - 1), 4, 4**q_1),
        )
        ...
        for i in range(4):
            for j in range(4):
                reduced_bell_densitymatrix[i, j] = self._densitymatrix[0, i, 0, j, 0] * 2 ** (
    ...
    def _plot_reduced_bell_basis(self, reduced_bell_densitymatrix):
        ...
        width = 0.5
        depth = 0.5
        values = np.linspace(0.2, 1.0, x.ravel().shape[0])
    ...
    def _validate_measure(self, insts):
        ...
        if set_flag:
            logger.warning(
                "No basis choice provided for measurement. Default value set to Pauli Z [Computational Basis]"
            )
    ...
    def run_experiment(self, experiment):
        ...
        for clock in range(levels):
                ...
                # Check if measure
                    ...
                    if sngl_measure:
                        if str(params[0]) == "X":
                            ...
                            ) = self._add_bell_basis_measure(
                                int(str(params[1])[0]),
                                int(str(params[1])[1]),
                                err_param=self._error_params["measurement_bell"],
                            )
                            data[
                                f"bell_probabilities{str(params[1])[0]}{str(params[1])[1]}"
                            ] = bell_probabilities
                            data[
                                f"reduced_bell_densitymatrix{str(params[1])[0]}{str(params[1])[1]}"
                            ] = reduced_bell_densitymatrix
                        else:
                        ...
                        data["Pauli_string_expectation"] = self._pauli_string_expectation(
                            str(params[1]), self._error_params["measurement"]
                        )
                        break
                        ...
                        prob, max_str, max_prob = self._add_ensemble_measure(
                            basis, add_param, self._error_params["measurement"]
                        )
            ...
            if self._depolarization_applied is True:
                self._add_depolarization_error(
                    clock,
    ...
    def _compute_densitymatrix(self, dmpauli):
        ...
        binary_index_value = [
            2 ** (self._number_of_qubits - i - 1) for i in range(self._number_of_qubits)
        ]
        ...
        for idxb in ind_mu:
            ...
            final_index = tuple(
                sum([binary_index_value[i] * index_list[i] for i in range(self._number_of_qubits)])
            )
        ...
        for current in range(len(partition)):
            ...
            for instruction in range(len(current_partition)):
                ...
                if name == "u1":
                    ...
                    print(self._two_qubit_gate_descriptor, "   qubit", qubit)
                if name == "measure":
